...
var (
	govDraftFieldNames          = builder.RawFieldNames(&GovDraft{})
	govDraftRows                = strings.Join(govDraftFieldNames, ",")
	govDraftRowsExpectAutoSet   = strings.Join(stringx.Remove(govDraftFieldNames, "`id`", "`create_at`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`", "`update_time`"), ",")
	govDraftRowsWithPlaceHolder = strings.Join(stringx.Remove(govDraftFieldNames, "`id`", "`create_at`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`", "`update_time`"), "=?,") + "=?"
)

type (
	govDraftModel interface {
		Insert(ctx context.Context, data *GovDraft) (sql.Result, error)
		FindOne(ctx context.Context, id int64) (*GovDraft, error)
		Update(ctx context.Context, data *GovDraft) error
		Delete(ctx context.Context, id int64) error
	}

	defaultGovDraftModel struct {
		conn  sqlx.SqlConn
		table string
	}
	...
	GovDraft struct {
		...
		ContentEdited       sql.NullString `db:"content_edited"`        // 修改后的内容 text1 自助机 text2 app
		...
		IsSaveDraft         sql.NullInt64  `db:"is_save_draft"`         // 1.已保存草稿
	}
)

func newGovDraftModel(conn sqlx.SqlConn) *defaultGovDraftModel {
	return &defaultGovDraftModel{
		conn:  conn,
		table: "`gov_draft`",
	}
}

func (m *defaultGovDraftModel) Delete(ctx context.Context, id int64) error {
	query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
	_, err := m.conn.ExecCtx(ctx, query, id)
	return err
}

func (m *defaultGovDraftModel) FindOne(ctx context.Context, id int64) (*GovDraft, error) {
	query := fmt.Sprintf("select %s from %s where `id` = ? limit 1", govDraftRows, m.table)
	var resp GovDraft
